...
class Data_Streamer():
    ...
    def stream_data(self):
        ...
        with open (init_object.data_path + init_object.streaming_file, 'r') as stream_file:
            for line in stream_file.readlines():
                x_val = line.strip('\n')
                print (x_val)
                producer_object.publish_message(producer_instance, "HousePrice", "data", str(x_val))
                time.sleep(2)
